...
class uci_electricity(torchdata.Dataset):#自定义数据集
    ...
    def get_data(self):
        ...
        for i in range(len(df.columns)):#假设为2列——————看不懂这个for循环！！！
            idx = np.flatnonzero(df.iloc[:, i])#返回df(假设df为5行2列的数据)中第i列非0元素的其他元素下标索引[[第一列结果],[第二列结果]](2行5列)
            arr_online[idx, i] = 1#arr_online为
        ...
        # Stack (and recreate Dataframe because stupid Pandas creates a Series otherwise)堆栈(并重新创建数据帧，因为pandas创建了一个系列)
        df = pd.DataFrame(df.stack())
        # Add time-based covariates添加基于时间的协变量
        df['Series'] = df.index.get_level_values(2)#返回请求级别的值索引
        df['Series'] = df['Series'].str.replace(r'MT_','').astype('int') - 1 #替换（去掉）MT_并转为数字
        # Add scaled time-based features添加缩放的基于时间的要素
        ...
        # Rename target column重命名目标列
        df.rename(columns={0:'E_consumption'}, inplace=True)
        # Add lagged output variable添加滞后输出变量
        df['E_consumption_lag'] = df.groupby(level=2)['E_consumption'].shift(1)#数据下移一位
        # Remove first date (contains NaNs for the lagged column)删除第一个日期(包含滞后列的NaNs)
        df = df.iloc[370:]        
        # Sort by series按系列排序
        df.index.names = ['date','hour','series']
        df.sort_index(level=['series','date','hour'], inplace=True)
        # Create feature matrix X and output vector Y创建特征矩阵X和输出向量Y
        df_Y = df[['E_consumption']]
        df.drop(['E_consumption'], axis=1, inplace=True)#删除'E_consumption'列
        df_X = df
        # Convert dataframe to numpy and reshape to [series x dates x features] format
        # 将dataframe转换为numpy，并重新整形为[series x dates x features]格式
        ...
        # Input and output dimensions输入和输出尺寸
        self.dim_input, self.dim_output = X.shape[-1], Y.shape[-1]
        # Convert to torch转换为pytorch
        ...
        # Create train, validate and test sets创建训练集、验证集和测试集
